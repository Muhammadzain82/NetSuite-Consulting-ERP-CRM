...
export default function BlogDetail({ params }) {
  ...
  useEffect(() => {
    const fetchBlogDetail = async () => {
      try {
        setLoading(true);
        // First fetch all blogs to find the one with matching title slug
        const response = await fetch(
        ...
        // Find the blog where the title matches the slug
        const foundBlog = blogs.find(blog => {
          const blogSlug = blog.attributes.title
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, '-')
            .replace(/(^-|-$)/g, '');
        ...
        // Now fetch the full details of the found blog using its ID
        const detailResponse = await fetch(
          `https://api.360xpertsolutions.com/api/blog-pages/${foundBlog.id}`
        );
  ...
  const renderContent = (contentItem, index) => {
    if (contentItem.type === "image" && contentItem.image) {
      const imageUrl = contentItem.image.url.startsWith("http")
        ? contentItem.image.url
        : `https://api.360xpertsolutions.com${contentItem.image.url}`;
